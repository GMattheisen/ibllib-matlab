...
%
function original_path = OPIvOr(dataset_type, original_paths, user, m, session)
...
for k = 1:length(original_paths)
    ...
    elseif contains(dataset_type, 'Settings')
        if contains(dataset_type,'computer')
            if contains(original_paths(k), 'computer')
                path = original_paths(k)
            end
        elseif contains(dataset_type, 'trial')
            if contains(original_paths(k), 'trial')
                path = original_paths(k)
            end
        elseif contains(dataset_type, 'session')
            if contains(original_paths(k), 'session')
                path = original_paths(k)
            end
        end
