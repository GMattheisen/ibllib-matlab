% =========================================================
% Find Original Paths for MoHa
%
function original_path = OPMoHa(dataset_type, original_paths, user, m)
    ...
    elseif contains(dataset_type,'_mflab_experimentSettings')
        index0 = find(contains(original_paths, '.mat'));
        tag_list = []; path_array = {};
        for k = 1:length(index0)
            OP = original_paths(index0(k));
            path_array{k,1} = OP{1,1};
            test = strsplit(string(original_paths(index0(k))), "/");
            file = strsplit(test(end),'_');
            tag_end = strsplit(file(end),'.');
            tag = tag_end(end-1);
            tag_list(end + 1) = str2double(tag);
        end    
        tag_list = sort(tag_list);
        if contains(dataset_type,'1')
            path_index = find(contains(path_array, int2str(tag_list(1))));
        elseif contains(dataset_type,'2')
            path_index = find(contains(path_array, int2str(tag_list(2))));
        elseif contains(dataset_type,'3')
            path_index = find(contains(path_array, int2str(tag_list(3))));
        end
        path = path_array{path_index,1};
        split_str = strsplit(path, "/");
        str_wo_mount = strjoin(split_str([4:end]),"/");
        original_path = "/mnt/" + user + "/" + str_wo_mount;           
        D.original_path{m} = original_path;
    else
        if contains(dataset_type,'computerSettings')
            index0 = find(contains(original_paths, 'computer_settings'));
        elseif contains(dataset_type,'sessionSettings')
            index0 = find(contains(original_paths, 'session_settings'));
        elseif contains(dataset_type,'trialSettings')
            index0 = find(contains(original_paths, 'trials'));
        end
        tag_list = []; path_array = {};
        if contains(dataset_type,'computerSettings') || contains(dataset_type,'sessionSettings')
            for k = 1:length(index0)
                OP = original_paths(index0(k));
                path_array{k,1} = OP{1,1};
                test = strsplit(string(original_paths(index0(k))), "/");
                file = strsplit(test(end),'_');
                tag = strsplit(file(end-2),'.');
                tag_list(end + 1) = str2double(tag);
            end    
        else contains(dataset_type,'trialSettings')
            for k = 1:length(index0)
                OP = original_paths(index0(k));
                path_array{k,1} = OP{1,1};
                test = strsplit(string(original_paths(index0(k))), "/");
                file = strsplit(test(end),'_');
                tag = strsplit(file(end-1),'.'); 
                tag_list(end + 1) = str2double(tag);
            end    
        end
        if contains(dataset_type,'1')
