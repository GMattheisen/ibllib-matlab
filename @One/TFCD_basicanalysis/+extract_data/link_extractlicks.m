function [AllLicksON] = link_extractlicks(p)
lickthresh = 0.1;
try 
    [DAQdata] = extract_data.link_getDAQDATA(p.eid);
    load([p.mat_metadata],'fsm','experiment_settings');
catch
    error('No DAQDATA_ and/or STIM_ files found.')    
end
Stim1ON = DAQdata.Stim1ON;
Stim1OFF = DAQdata.Stim1OFF;
Stim2ON = DAQdata.Stim2ON;
Stim2OFF = DAQdata.Stim2OFF;
Valve = DAQdata.Valve;
Valve2 = DAQdata.Valve2;
Lick = DAQdata.Lick;
Lick2 = DAQdata.Lick2;
...
%     if ~isempty(IdxON)
%         cla
%         plot(LickTrace), hold on, plot(licksout(activetrials(cT)), treshLick,'rx')
%         xlim([licksout(activetrials(cT))-1000 licksout(activetrials(cT))+1000])
%         pause(3)   
%     end
end 
end
